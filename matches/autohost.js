	...
	async execute(bancho, osuUserId, settings) {
		...
		for (let i = 0; i < 5; i++) {
			try {
				...
				if (i === 4) {
					if (settings.interaction) {
						await DBElitebotixProcessQueue.create({ guildId: 'None', task: 'interactionResponse', additions: `${settings.interaction};${settings.messageId};I am having issues creating the lobby and the match has been aborted. Please try again later.`, priority: 1, date: new Date() });
					}
